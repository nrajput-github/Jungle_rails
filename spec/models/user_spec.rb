...
RSpec.describe User, type: :model do
  ...
  subject {described_class.new(:first_name => 'Colin', :last_name => 'Jones', :email => 'colin@gamil.com', :password => "2424", :password_confirmation => "2424", :password_digest => "123")}
  ...
  describe 'Validations do' do
    ...
    it 'fails to save when password and password_confirmation does not match' do
      subject.password_confirmation = '333'
      subject.valid?
      expect(subject.errors).not_to be_empty
    end

    it 'fails to save when email is not unique (not case sensitive)' do
      User.create(:first_name => 'Colin', :last_name => 'Jones', :email => 'colin@gamil.com', :password => "2424", :password_confirmation => "2424", :password_digest => "123")
      subject.valid?
      expect(subject.errors).not_to be_empty
    end
